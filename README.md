...
We are given two csv format type files. There is sales order data based on a product in `order_products.csv` file. From this file, I extract total number of sales per product and respective first time order count. In the second file, products are categorized by departments. Since I have total number of sales per product from the first file, I sum total number of sales of all products belonging to the same department.  
